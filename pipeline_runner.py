...
def patch_doping_script(cfg):
    ...
    text = replace_assignment(text, "INPUT_CIF", py_repr(str(ROOT / cfg["cif"])))
...
def patch_rdf_script(cfg):
    ...
    temps = cfg.get("md_temperatures", [900]) or [900]
    ...
    if pairs == "all":
        ...
        pair_files = [f"{p}.dat" if not str(p).endswith(".dat") else str(p) for p in pairs]
        pair_expr = "dat_files=(" + " ".join(shell_quote(p) for p in pair_files) + ")"
...
def run_pipeline(cfg):
    ...
    if stages.get("summary", True):
        run_command("summary", [sys.executable, str(ROOT / "summary_writer.py")])
...
def main(argv=None):
    ...
    cfg = load_config(ROOT / args.input)
